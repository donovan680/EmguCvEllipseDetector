...
namespace Yaed
{
	...
	class CEllipseDetectorYaed
	{
		...
		// Selection strategy - Step 1 - Discard noisy or straight arcs. See Sect [3.1.2] in the paper
		int		_iMinEdgeLength;					// minimum edge size (ThLength)				
		float	_fMinOrientedRectSide;				// minumum size of the oriented bounding box containing the arc (ThOBB)
		float	_fMaxRectAxesRatio;					// maximum aspect ratio of the oriented bounding box containing the arc

		// Selection strategy - Step 2 - Remove according to mutual convexities. See Sect [3.2.1] in the paper
		float _fThPosition;

		// Selection Strategy - Step 3 - Number of points considered for slope estimation when estimating the center. See Sect [3.2.1] in the paper
		unsigned _uNs;									// Find at most Ns parallel chords.

		// Selection strategy - Step 3 - Discard pairs of arcs if their estimated center is not close enough. See Sect [3.2.2] in the paper
		...
		// Validation - Points within a this threshold are considered to lie on the ellipse contour. See Sect [3.3.1] in the paper
		float	_fDistanceToEllipseContour;			// maximum distance between a point and the contour. See equation [] in the paper

		// Validation - Assign a score. See Sect [3.3.1] in the paper
		float	_fMinScore;							// minimum score to confirm a detection Eq.[25]
